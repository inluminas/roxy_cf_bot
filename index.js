...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'register') {
        ...
        if (!match) {
            return interaction.reply('Invalid Codeforces profile URL. Please provide a valid one.');
        }
        ...
        await interaction.reply(`Please add the following code to your Codeforces bio: \`${verificationCode}\`, then use the \`/verify\` command to confirm.`, { ephemeral: true});
    ...
    if (commandName === 'verify') {
        ...
        if (!users[discordUsername] || !users[discordUsername].verificationCode) {
            return interaction.reply('You haven\'t started the registration process. Use `/register` first.');
        }
        ...
        try {
            ...
            if (bio && bio.includes(verificationCode)) {
                ...
                let role = interaction.guild.roles.cache.find(r => r.name === getCfRole(rating));
                if (role) {
                    await interaction.member.roles.add(role);
                }
                
                await interaction.reply(`Verification successful! Your handle ${handle} has been linked. Role assigned: ${getCfRole(rating)}`);
            } else {
                await interaction.reply('Verification failed. Please ensure the code is in your profile bio.');
            }
        } catch (error) {
            console.error('Error fetching Codeforces profile:', error);
            await interaction.reply('An error occurred while fetching your Codeforces profile. Please try again.');
        }
    }
    ...
    if (commandName === 'update') {
        ...
        if (!users[discordUsername] || !users[discordUsername].handle) {
            return interaction.reply('You have not registered yet. Use `/register` first.');
        }
        ...
        if (rating !== null) {
            await interaction.reply(`Your profile has been updated! Current rating: ${rating}`);
        } else {
    ...
    if (commandName === 'profile') {
        ...
        const response = await axios.get(`${CODEFORCES_API_USER}${handle}`);
        const userProfile = response.data.result[0];
        const rating = userProfile.rating || 'Unrated';
        const friendsOf = userProfile.friendOfCount || 'N/A';

        const embed = new EmbedBuilder()
            .setTitle(`${userProfile.handle}'s Profile`)
            .setURL(`https://codeforces.com/profile/${handle}`)
            .setColor(0x00AE86)
            .addFields(
                { name: 'Handle', value: userProfile.handle, inline: true },
                { name: 'Rating', value: rating.toString(), inline: true },
                { name: 'Friends of', value: friendsOf.toString(), inline: true }
            )
            .setThumbnail('https://sta.codeforces.com/s/78255/images/codeforces-logo-with-telegram.png');
    ...
    try {
        ...
        await rest.put(
            ...
            { body: [
                ...
                {
                    name: 'verify',
                    description: 'Verify your Codeforces profile by checking bio for verification code',
                },
                {
                    name: 'update',
